...
useLayoutEffect(()=>{
   ...
   if(pannelWidth+pannelLeft>pannelWidth){
    ...
    return <Box ref={ref} className={`shadow-lg absolute bg-white p-5 text-black ${top?'top-full':'bottom-full'} ${left?'left-0':'right-0'} z-10`} style={{minWidth:'40vw'}}>
...
<List dense className={'p-0'}>
    {acepciones && acepciones.map((el,index)=>{
